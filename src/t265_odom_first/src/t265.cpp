...
MyT265Camera::MyT265Camera(ros::NodeHandle& node_hanle,
                           ...
                           _nh(node_hanle), _pnh(private_node_handle),
...
rs2::device MyT265Camera::getDevice() {
    ...
    ROS_INFO_STREAM("device lise size is " << device_list.size());
...
void MyT265Camera::setupOdomPub() {
    ...
    _sensor.open(_stream_profile);
    _sensor.start(&MyT265Camera::poseCallBack);
}

void MyT265Camera::poseCallBack(rs2::frame frame) {
    ROS_INFO("start pose callback function");
